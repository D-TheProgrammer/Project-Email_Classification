...
# Fonction pour détecter l'interpréteur Python disponible (python ou python3)
def trouver_executable_python():
    try:
        ...
        try:
            ...
            print("Aucun interpréteur Python n'est trouvé.")
            sys.exit(1)
...
def train_model(model_script):
    ...
    # Récupérer l'exécutable Python correct (python ou python3)
    python_executable = trouver_executable_python()

    # Ajout du chemin absolu de 'src' au PYTHONPATH
    env = os.environ.copy()
    env['PYTHONPATH'] = os.path.abspath('src')

    # Démarrer le fichier avec le bon interpréteur Python
    subprocess.check_call([python_executable, os.path.join('src', 'modeling', model_script)], env=env)
...
# Pour lancer les modèles correspondants 
def main():
